...
function ResultCard({ result, query }) {
  ...
  return (
    <div className="rounded-2xl overflow-hidden" style={{
      ...
      {/* Top answer strip */}
      <div className="p-6 border-b theme-border" style={{ background: 'linear-gradient(135deg, var(--bg-active-faint), var(--bg-faint))' }}>
        <div className="flex items-start justify-between gap-4 mb-4">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0"
              style={{ background: 'var(--accent-gradient)', boxShadow: '0 0 16px var(--accent-shadow)' }}>
              <Sparkles className="w-4 h-4 text-white" />
            </div>
            <div>
              <p className="text-xs font-black uppercase tracking-wider theme-accent-text">Antigravity AI Answer</p>
              <p className="text-xs theme-text-muted opacity-80">{result.mode === 'deep' ? 'Deep Research Mode' : 'Quick Answer'}</p>
            </div>
          </div>
          <div className="flex items-center gap-2 flex-shrink-0">
            <button onClick={copy} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all hover:scale-105"
              style={{ background: copied ? 'var(--bg-success-faint)' : 'var(--bg-faint)', color: copied ? 'var(--text-success)' : 'var(--text-faint-muted)', border: `1px solid ${copied ? 'var(--border-success)' : 'var(--border-faint)'}` }}>
              {copied ? <><Check className="w-3 h-3" /> Copied</> : <><Copy className="w-3 h-3" /> Copy</>}
            </button>
          </div>
        </div>

        <div className="prose theme-text prose-sm max-w-none leading-relaxed" style={{ maxHeight: expanded ? 'none' : '280px', overflow: 'hidden' }}>
          ...
          <div className="space-y-1.5">
            {result.followUps.map((q, i) => (
              <button key={i} onClick={() => result.onFollowUp(q)}
                ...
                <ChevronRight className="w-4 h-4 flex-shrink-0 theme-text-muted group-hover:theme-accent-text transition-colors" />
...
export default function Antigravity() {
  ...
  const modes = [
    { id: 'quick', icon: Zap, label: 'Quick', color: 'var(--accent-color)', desc: 'Fast, concise answer' },
    { id: 'deep', icon: Brain, label: 'Deep Research', color: 'var(--accent-color)', desc: 'Multi-paragraph analysis' },
    { id: 'explain', icon: Lightbulb, label: 'Explain Simply', color: 'var(--accent-color)', desc: 'Simple language' },
    { id: 'quiz', icon: HelpCircle, label: 'Quiz Me', color: 'var(--accent-color)', desc: 'Turn it into a quiz' },
  ];

  const suggestions = [
    { icon: Atom, text: 'How does quantum entanglement work?', color: 'var(--accent-color)' },
    { icon: BookOpen, text: 'Causes and effects of World War I', color: 'var(--accent-color)' },
    { icon: FlaskConical, text: 'Explain CRISPR gene editing', color: 'var(--accent-color)' },
    { icon: Globe, text: 'What is the prisoner\'s dilemma?', color: 'var(--accent-color)' },
    { icon: Brain, text: 'How does long-term memory work?', color: 'var(--accent-color)' },
    { icon: Star, text: 'Explain the Drake equation', color: 'var(--accent-color)' },
  ];
    ...
    <div className="min-h-screen theme-bg theme-text overflow-x-hidden">
      <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden">
        <div className="absolute w-[700px] h-[700px] rounded-full -top-40 -left-40"
          style={{ background: 'radial-gradient(circle, var(--accent-color-transparent) 0%, transparent 70%)', filter: 'blur(60px)' }} />
      </div>
      ...
      <div className="relative z-10 max-w-3xl mx-auto px-4 py-10 space-y-8">
        ...
        {isFirstSearch && (
          <div className="text-center space-y-4 pt-12 pb-6" style={{ animation: 'fadeInUp 0.5s ease' }}>
            <div className="w-20 h-20 mx-auto rounded-3xl flex items-center justify-center"
              style={{ background: 'var(--accent-gradient)', boxShadow: '0 0 50px var(--accent-shadow)' }}>
              <Atom className="w-10 h-10 text-white" style={{ animation: 'spin-slow 8s linear infinite' }} />
            </div>
            <div>
              <h1 className="text-4xl md:text-5xl font-black theme-accent-text mb-2">Antigravity</h1>
        ...
        <div className="space-y-3">
          ...
          <div className="relative">
            <div className="flex items-center gap-3 rounded-2xl p-3 transition-all"
              style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-active)', boxShadow: '0 0 30px var(--accent-shadow-light)' }}>
              <Search className="w-5 h-5 flex-shrink-0 theme-text-muted" />
              <input
                ref={inputRef}
                type="text"
                value={query}
        ...
        {isFirstSearch && (
          <div className="space-y-3">
            <p className="text-xs font-black uppercase tracking-wider text-center theme-text-muted opacity-30">Try asking</p>
